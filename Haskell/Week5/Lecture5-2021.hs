...
exam program expansion n d filename = 
  do
    let totalFractal = times n (refine expansion) $ program :: Fractal
    writeSVG filename (concretize d totalFractal)

programT =
  fstep >-> fturn (-120) >-> fstep >-> fturn (-120) >-> fstep
expansionT =
  fstep >-> fturn (60) >-> fstep >-> fturn (-120) >-> fstep >-> fturn (60) >-> fstep


--TEST 
--exam programT expansionT 2 1 "outputTest.txt"