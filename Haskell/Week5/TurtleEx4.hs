...
boundingBox (((x1,y1),(x2,y2)):lines) =  ((minX,minY),(maxX,maxY))
    where ((smallX,smallY),(bigX,bigY)) = boundingBox lines
          (maxX,maxY) = (max (max x1 x2) bigX , max (max y1 y2) bigY)
          (minX,minY) = (min (max x1 x2) bigX , min (max y1 y2) bigY)
